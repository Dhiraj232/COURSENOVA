...
const EXAMS = [
    { title: 'JEE Main Mock Test Series', id: 'jee-main-national-free' },
    { title: 'NEET Mock Test Series', id: 'neet-national-free' },
    { title: 'CUET UG Mock Test Series', id: 'cuet-ug-national-free' },
    { title: 'CUET PG Mock Test Series', id: 'cuet-pg-national-free' },
    { title: 'BHU Entrance Mock Test Series', id: 'bhu-national-free' },
    { title: 'AMU Entrance Mock Test Series', id: 'amu-national-free' },
    { title: 'NDA Mock Test Series', id: 'nda-national-free' }
];
...
async function seedData() {
    try {
        ...
        console.log("Cleaning up all tests in the TRUE National Exams section...");
        
        await MockTestPack.deleteMany({ category: 'National Level Exam' });
        
        const titlesToClear = EXAMS.map(e => e.title);
        await PracticeQuestion.deleteMany({ category: { $in: titlesToClear }, isMockTestOnly: true, difficulty: 'Hard' });

        console.log("Creating brand new Free True National Entrance Exam Mock Tests...");
        ...
        for (const exam of EXAMS) {
            ...
            const packInfo = {
                id: exam.id,
                title: exam.title,
                category: 'National Level Exam', // Maps to the new trueNationalTestsSection in the UI
                description: `Official free full-length mock test series for ${exam.title}. Contains 3 complete sets of exactly 70 mixed bilingual questions.`,
                thumbnail: 'https://placehold.co/400x200?text=National+Level',
                price: 0,
                isFree: true,
                totalTests: 3,
                tests: [],
        ...
        console.log("\n🎉 True National Exam Seeding Completed Successfully!");
        process.exit(0);

    } catch (error) {
        console.error("Error seeding True National Exams:", error);
        process.exit(1);
    }
}
